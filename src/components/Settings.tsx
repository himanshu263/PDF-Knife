...
const ToggleSwitch = ({ checked, onChange }: { checked: boolean, onChange: () => void }) => (
  <button 
    ...
    className={`w-12 h-7 rounded-full p-1 transition-all duration-300 ${checked ? 'bg-blue-600 shadow-lg shadow-blue-600/20' : 'bg-gray-200 dark:bg-zinc-700'}`}
  >
  ...
  return (
    ...
    >
      <div className="flex items-center gap-4 flex-1 overflow-hidden">
        <div className={`w-10 h-10 rounded-2xl flex items-center justify-center shrink-0 transition-colors ${danger ? 'bg-red-50 dark:bg-red-900/20 text-red-500' : (iconColor || 'bg-gray-100 dark:bg-zinc-800 text-gray-500 group-hover:text-blue-600 group-hover:bg-blue-50 dark:group-hover:bg-blue-900/20')}`}>
    ...
    <NativeToolLayout title="System" description="Core Configuration" actions={null}>
      <div className="animate-in fade-in slide-in-from-bottom-2 duration-500 pb-40">
        ...
        <div className="flex items-center gap-4 px-2 mb-8 mt-2">
           <div className="w-12 h-12 bg-blue-600 rounded-2xl flex items-center justify-center shadow-lg shadow-blue-600/20 text-white shrink-0">
        ...
        <SettingGroup title="Workflow">
          ...
          <div className="p-5 flex flex-col gap-3">
            ...
            <input 
              ...
              placeholder="e.g. himanshu263"
              className="w-full bg-gray-100 dark:bg-black border border-transparent focus:border-blue-600 rounded-xl px-4 py-3.5 text-xs font-black outline-none transition-all placeholder:text-gray-400 dark:text-white"
            />
          </div>
        </SettingGroup>
        ...
        <SettingGroup title="Privacy">
          ...
          {autoWipe && (
            <div className="px-5 py-3 flex items-center justify-between bg-blue-50/50 dark:bg-blue-900/10 border-t border-blue-100/20 dark:border-blue-900/20 animate-in slide-in-from-top-2">
               <span className="text-[9px] font-black text-blue-600 uppercase tracking-widest">Wipe Delay</span>
               <select 
                value={wipeTimer}
                onChange={(e) => handleSelect('autoWipeTimer', e.target.value, setWipeTimer)}
                className="bg-transparent text-[11px] font-black text-blue-700 outline-none cursor-pointer text-right"
               >
          ...
          <div className="px-5 py-4 flex items-center justify-between">
             ...
             <select 
              ...
              className="bg-gray-100 dark:bg-black px-3 py-2 rounded-xl text-[11px] font-black text-gray-600 dark:text-gray-300 outline-none border border-transparent focus:border-blue-600 cursor-pointer"
             >
